...
import { Calendar, Users, Settings, BarChart3, Clock, Mail, Phone, RefreshCw, Send, Filter } from 'lucide-react';
...
export const AdminDashboard: React.FC = () => {
  ...
  const sendReminder = async (booking: BookingWithService) => {
    try {
      ...
      if (response.ok) {
        alert(`✅ Herinnering succesvol verzonden naar ${booking.customer_email}`);
      } else {
        throw new Error(result.error || 'Email versturen mislukt');
      }
    } catch (error) {
      console.error('Reminder error:', error);
      alert('❌ Kon herinnering niet versturen. Check de console voor details.');
    }
  };

  const getStatusColor = (status: string) => {
    switch (status) {
      case 'confirmed': return 'text-green-500 bg-green-500/20 border-green-500/50';
      case 'cancelled': return 'text-red-500 bg-red-500/20 border-red-500/50';
      case 'completed': return 'text-blue-500 bg-blue-500/20 border-blue-500/50';
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 md:mb-12 gap-4">
          <div>
            ...
            <p className="text-bariq-grey text-sm md:text-base">Beheer je boekingen en klanten</p>
          </div>
          <button
              ...
              {(['all', 'today', 'upcoming', 'past'] as const).map((filterOption) => (
                ...
                >
                  {filterOption === 'all' ? 'Alle' : 
                   filterOption === 'today' ? 'Vandaag' :
                   filterOption === 'upcoming' ? 'Komende' : 'Verleden'}
                </button>
              ))}
            </div>
          </div>
          <div className="flex flex-wrap gap-2 pl-8">
            {(['all', 'confirmed', 'completed', 'cancelled'] as const).map((statusOption) => (
              <button
                key={statusOption}
              ...
              >
                {statusOption === 'all' ? 'Alle Status' : getStatusLabel(statusOption)}
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4 mb-8">
          <div className={cardClass}>
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
              <div>
                <p className="text-bariq-grey text-xs md:text-sm mb-1">Vandaag</p>
          ...
          <div className={cardClass}>
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
              <div>
                <p className="text-bariq-grey text-xs md:text-sm mb-1">Bevestigd</p>
          ...
          <div className={cardClass}>
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
              <div>
                <p className="text-bariq-grey text-xs md:text-sm mb-1">Voltooid</p>
        ...
        <div className={cardClass}>
          <div className="flex items-center gap-3 mb-6">
            ...
            <h2 className="text-xl md:text-2xl font-display font-semibold text-bariq-white">
              Boekingen <span className="text-bariq-grey">({bookings.length})</span>
            </h2>
          </div>
          {loading ? (
              ...
              <table className="w-full min-w-[800px]">
                <thead>
                  <tr className="border-b border-gray-800">
                    <th className="text-left py-3 px-3 md:px-4 font-semibold text-bariq-grey text-sm">Klant</th>
                    <th className="text-left py-3 px-3 md:px-4 font-semibold text-bariq-grey text-sm">Service</th>
                    <th className="text-left py-3 px-3 md:px-4 font-semibold text-bariq-grey text-sm">Datum & Tijd</th>
                    <th className="text-left py-3 px-3 md:px-4 font-semibold text-bariq-grey text-sm">Status</th>
                    <th className="text-left py-3 px-3 md:px-4 font-semibold text-bariq-grey text-sm">Acties</th>
                  </tr>
                </thead>
                <tbody>
                  {bookings.map((booking) => (
                    <tr key={booking.id} className="border-b border-gray-800 hover:bg-gray-800/30 transition-colors">
                      <td className="py-4 px-3 md:px-4">
                        <div>
                          <p className="font-semibold text-bariq-white text-sm md:text-base">{booking.customer_name}</p>
                      ...
                      <td className="py-4 px-3 md:px-4">
                        <div>
                          <p className="font-semibold text-bariq-white text-sm md:text-base">{booking.service?.name || 'Onbekend'}</p>
                          <div className="flex items-center text-xs md:text-sm text-bariq-grey mt-1">
                      ...
                      <td className="py-4 px-3 md:px-4">
                        <span className={`px-2 md:px-3 py-1 rounded-full text-xs font-semibold border ${getStatusColor(booking.status)}`}>
