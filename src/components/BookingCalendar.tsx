...
export const BookingCalendar: React.FC<BookingCalendarProps> = ({
  ...
  const handleBooking = async () => {
    ...
    const mainService = mainServices.find(
      (s) => s.category === selectedVehicleClass
    );
    if (!mainService) {
    ...
    if (conflictingBookings && conflictingBookings.length > 0) {
      setError(
        "⚠️ Sorry! Dit tijdslot is net geboekt door iemand anders. Kies een ander tijdslot."
      );
      setLoading(false);
    ...
    if (bookingError) {
      ...
      if (
        ...
        setError(
          "⚠️ Dit tijdslot is al bezet. Kies alstublieft een ander tijdslot."
        );
        // Herlaad beschikbare slots
        loadAvailableSlots(selectedDate);
      } else {
        setError(
          "Er ging iets mis bij het maken van de boeking. Probeer het opnieuw."
        );
      }
  ...
  const checkLicensePlate = async () => {
    ...
    try {
      ...
      if (data && data.length > 0) {
        ...
        setError(
          "Kenteken niet gevonden. Selecteer handmatig je voertuigklasse."
        );
      }
    } catch (err) {
      console.error("RDW API Error:", err);
      setError(
        "Kon kenteken niet controleren. Selecteer handmatig je voertuigklasse."
      );
    } finally {
      ...
      <div className={containerClass}>
        <div className="max-w-2xl mx-auto pt-8 md:pt-12">
          <div className={`${cardClass} text-center`}>
            ...
            <button onClick={resetBooking} className={buttonPrimaryClass}>
        ...
        {!selectedVehicleClass && (
          <div className={cardClass}>
            <div className="mb-6 md:mb-8">
              ...
              <p className="text-bariq-grey text-sm md:text-base ml-9">
                Weet je het niet zeker? Gebruik de kenteken check hieronder.
              </p>
            </div>
            ...
            <div className="grid md:grid-cols-2 gap-5 md:gap-6 mb-6">
              ...
              >
                ...
                <div className="text-bariq-red font-bold text-4xl md:text-5xl mb-6">
                  €90
                </div>
                <p className="text-bariq-grey text-sm font-medium mb-3 border-t border-gray-800 pt-4">
                  Voorbeelden:
                </p>
            ...
            >
              ...
              {showLicensePlateCheck
                ? "Verberg kenteken check"
                : "Weet je het niet? Doe een kenteken check"}
            </button>

            {showLicensePlateCheck && (
              <div className="mt-6 bg-bariq-black-lighter rounded-xl p-6 md:p-7 border border-gray-800">
                <h4 className="font-display font-semibold text-lg md:text-xl mb-2 text-bariq-white">
                  Kenteken Check
                </h4>
                ...
                <div className="mt-4 flex items-start gap-2 bg-gray-800/50 rounded-lg p-3 border border-gray-700">
                  <svg
                    className="w-4 h-4 text-bariq-grey flex-shrink-0 mt-0.5"
                    fill="currentColor"
                    viewBox="0 0 20 20"
                  >
                    <path
        ...
        {selectedVehicleClass && !addonsCompleted && (
          <div className={cardClass}>
            ...
            <div className="mb-6 md:mb-8">
              ...
              <p className="text-bariq-grey text-sm md:text-base ml-9">
                Optioneel - kies één of meerdere, of sla over
              </p>
            </div>
                  ...
                  >
                    <div className="flex justify-between items-start mb-4">
                      <div className="flex-1 pr-3">
                        ...
                        <p className="text-bariq-red font-bold text-2xl">
                          €{addon.price}
                        </p>
                      </div>
                      <div
                    ...
                    <p className="text-bariq-grey text-xs md:text-sm leading-relaxed line-clamp-3">
                      {addon.description}
                    </p>
                    {isSelected && (
            ...
            <div className="bg-bariq-black border-2 border-bariq-red rounded-xl p-6 md:p-7 mb-8">
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div className="flex-1">
                  <p className="text-bariq-grey text-sm md:text-base font-medium mb-1">
                    Totale prijs
                  </p>
                  <p className="text-bariq-white text-base md:text-lg font-semibold">
                    {getMainService()?.name}
                    {selectedAddons.length > 0 && (
                      <span className="text-bariq-red">
                        {" "}
        ...
        {addonsCompleted && !selectedDate && (
          <div className={cardClass}>
            ...
            <div className="mb-6 md:mb-8">
              ...
              <p className="text-bariq-grey text-sm md:text-base ml-9">
                Maandag gesloten • Dinsdag t/m Zondag 8:00-20:00
              </p>
            </div>
          ...
          <div className={cardClass}>
            ...
            {availableSlots.length > 0 ? (
              ...
              <div className="text-center py-8">
                <p className="text-bariq-grey mb-2">
        ...
        {selectedTime && (
          <div className={cardClass}>
            ...
            <div className="bg-bariq-black-lighter border border-gray-800 rounded-xl p-6 mb-8">
              <h3 className="font-display font-semibold text-lg mb-4 text-bariq-white">
                Overzicht van je boeking
              </h3>
              <div className="space-y-3 text-sm">
                <div className="flex justify-between py-2">
                  <span className="text-bariq-grey">Service:</span>
                  <span className="font-medium text-bariq-white">
                    {getMainService()?.name}
                  </span>
                </div>
                {selectedAddons.length > 0 && (
                  <div className="bg-gray-800/30 border border-gray-800 rounded-lg p-3 my-2">
                    <p className="text-bariq-grey text-sm mb-2 font-medium">
                      Extra's ({selectedAddons.length}):
                    </p>
                    {selectedAddons.map((addon) => (
                      <div
                        ...
                        className="flex justify-between text-sm py-1"
                      >
                        <span className="text-bariq-grey-light">
                          • {addon.name}
                        </span>
                        <span className="text-bariq-grey-light">
                          €{addon.price}
                ...
                <div className="flex justify-between border-t border-gray-800 pt-3 mt-3 items-center">
                  <span className="text-bariq-white font-semibold text-base">
                    Totaal:
                  </span>
                  <span className="font-bold text-bariq-red text-2xl">
                    €{getTotalPrice()}
                  </span>
                </div>
              </div>
